...
namespace SPIROC_DAQ
{
    public partial class Main_Form : Form
    {
        ...
        private CancellationTokenSource S_curveTks = new CancellationTokenSource();
        ...
        {
            S_curveTks.Dispose();       //clean up old token source
            S_curveTks = new CancellationTokenSource(); // generate a new token
            if (check_USB() == false)
            {
                MessageBox.Show("USB or Instrument is not connected", "Error");
                return;
            ...
            {
                Form2 paraWindows = new Form2();
                paraWindows.label1.Text = "DAC threshold value";
                paraWindows.label2.Text = "tunnel number ";
                paraWindows.ShowDialog(this);

                try
                {
                    Task voltageSweepTask = Task.Factory.StartNew(() => this.Scurve_sweep_threadFunc(S_curveTks.Token, paraWindows), S_curveTks.Token);
