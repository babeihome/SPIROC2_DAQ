...
namespace SPIROC_DAQ
{
    partial class Main_Form
    {
        ...
        private void Scurve_sweep_threadFunc(CancellationToken taskToken, Form2 paraWindows)
        {
            // extDelay is the duration that signal minus trigger
            // unit is nano-second
            ...
            uint tunnel_start = uint.Parse(paraWindows.start2.Text);
            uint tunnel_step = uint.Parse(paraWindows.step2.Text);
            uint tunnel_stop = uint.Parse(paraWindows.stop2.Text);
            ...
            DateTime dayStamp = DateTime.Now;
            string subDic = string.Format("{0:yyyyMMdd}_{0:HHmm}_DACtunnelSweep", dayStamp);
            string fullPath = folderBrowserDialog1.SelectedPath + '\\' + subDic;
            if (!Directory.Exists(fullPath))
                Directory.CreateDirectory(fullPath);
            ...
            // outside loop is for preamp gain
            for (uint DAC = DAC_start; DAC <= DAC_stop; DAC  += DAC_step)
            {
                if (taskToken.IsCancellationRequested == true)
                {
                    break;
                ...
                for (int chn = 0; chn < 36; chn++)
                {
                    string Key = "PREAMP_GAIN" + chn.ToString();
                    uint old_value = slowConfig.get_property(slowConfig.settings[Key.ToString()]);
                    uint new_value = (DAC << 2) + (old_value & 0x03);
                    slowConfig.set_property(slowConfig.settings[Key.ToString()], new_value);

                }
                // inside loop is for trig_delay
                for (uint tunnel = tunnel_start; tunnel <= tunnel_stop; tunnel  += tunnel_step)
                {
                    for (uint i = 0; i<=35; i++)
                    {
                        if (i == tunnel & i <=17 )
                        {
                            ...
                            slowConfig.set_property(slowConfig.settings["DISCRIMINATOR_MASK1"], Convert .ToUInt32  (Convert.ToString  ((int) k,2)));
                        }
                        else if (i == tunnel & i>=18)
                        {
                            slowConfig.set_property(slowConfig.settings["DELAY_TRIGGER"], 0);
                        }
                        else { slowConfig.set_property(slowConfig.settings["DELAY_TRIGGER"], 1); }

                    }
                   
                    sendMessage("Sweep point: \n\t tunnel of asic:\t" + tunnel .ToString() + "\n\t DAC:\t" + DAC .ToString() + "\n");
                    ...
                    Thread.Sleep(500);
                    //file name include voltage value;                  
                    string fileName = string.Format("preamp_{0:#0}xdelay_{1:#0}.dat", DAC, tunnel);
                    //create file writer
                    bw = new BinaryWriter(File.Open(fullPath + '\\' + fileName, FileMode.Create, FileAccess.Write, FileShare.Read));
